...
- `<prefix> + H`：打开 Language Coach 只读 `nvim` 历史视图，按日期/时间分组；可直接用 `nvim` 的搜索、跳转、折叠等能力，`q` 关闭
...
- `<prefix> + H` 会把 `~/.tmux-language-history` 渲染成只读 `nvim` 视图，按日期/时间分组显示 `IN / EN / NOTE / TIP / CONTEXT / BEST`；旧记录仍可读取。如果本机没有 `nvim`，会自动回退到 shell 历史查看器。
