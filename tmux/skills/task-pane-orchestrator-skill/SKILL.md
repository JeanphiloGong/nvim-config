...
into concrete tmux panes, pane_id routing, lane forks, and role-first startup
messages.
...
- deciding task status, task blockers, or phase policy for the whole window
...
1. Validate inputs and confirm the session is already inside the intended
   worktree.
2. Inspect repo state, current pane map, and the lane plan already chosen for
   the task window.
3. Detect tmux environment:
   - this skill assumes the bootstrap flow has already opened the task window
   - this skill does not own creation of the first task window
