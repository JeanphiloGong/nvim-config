...
- Mermaid LSP（外部依赖）：从 `https://github.com/thepwagner-org/mermaid-language-server` 克隆源码，执行 `npm install` 和 `npm run build`，并创建调用 `dist/server.js` 的 `~/.local/bin/mermaid-language-server` 启动器后，`.mmd` 文件会启用语法诊断（filetype `mermaid`）。
