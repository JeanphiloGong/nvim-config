...
- `:AgentBoard`：在 Neovim 中打开 tmux agent 编排器；左侧是 session/window/pane workspace tree，右侧按选中范围显示 scope inspector 或单 agent workspace，`Enter` / `Space` 展开或预览，`i` 可发送输入，`J` 可跳转。
