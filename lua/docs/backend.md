...
- `williamboman/mason-lspconfig.nvim`：将 Mason 安装结果桥接给 LSP，`ensure_installed = { pyright, svelte, ts_ls, gopls, clangd, rust_analyzer, solidity_ls_nomicfoundation, csharp_ls@0.15.0 }`，`automatic_enable = false` 避免自动 attach，由下方自定义的 `vim.lsp.enable` 负责启用。
- `neovim/nvim-lspconfig`：为各语言启动 LSP（含 Mermaid 自定义 `cmd`），自动注入 `cmp_nvim_lsp` 的补全能力；Solidity 使用 Nomic Foundation language server，避免旧服务器在每个项目中先下载远程 `latest` 编译器；内置 `with_capabilities` 辅助函数方便在新增服务器时保持一致配置。

## 快速上手
1. 运行 `:Lazy sync` 确保插件就绪。
2. 打开 `:Mason`，确认常用服务器已安装（如 `pyright`、`svelte-language-server`、`typescript-language-server`）；若缺少可在 UI 中按 `i` 安装。
3. Mermaid 语法辅助需从源码安装（该项目当前未发布 npm 包）：克隆 `https://github.com/thepwagner-org/mermaid-language-server`，执行 `npm install` 和 `npm run build`，再创建一个调用 `dist/server.js` 的 `~/.local/bin/mermaid-language-server` 启动器。
...
- 预设按键：在 Svelte buffer 中提供 `gd` 智能分屏打开定义（优先均衡分布，必要时再按宽高比兜底）、`gD` 定义列表、`K` 悬停、`<leader>rn` 重命名（其他语言可使用全局映射 `gd/gD/gr/gi` 调用 LSP）。
